        ...
        conn.connect(
            function(err) {
                if (err) {
                    ...
                    conn.query('CREATE TABLE IF NOT EXISTS containers_info(http_port INTEGER, ssh_port INTEGER, name TEXT, description TEXT, creator TEXT, gpu_address VARCHAR(15), on_off INTEGER, PRIMARY KEY (http_port, gpu_address))',
                        function(err, results, fields) {
                            if (err) throw err;
                        });
    ...
    addContainer: function(params) {
        ...
        conn.connect(function(err) {
            if (err) {
                ...
                conn.query("INSERT INTO containers_info(http_port, ssh_port, name, description, creator, gpu_address, on_off) VALUES(?, ?, ?, ?, ?, ?, ?)",
                            [params["http_port"], params["ssh_port"], params["containerName"], params["containerDes"], params["username"], params["gpu_address"], params["on_off"]],
                    function(err, results, fields) {
                        if (err) throw err;
                    })
                conn.end(function(err) {
    ...
    setContainerOff: function(containerName, gpu, callback) {
      ...
      conn.connect(function(err) {
          if (err) {
              ...
              conn.query("UPDATE containers_info SET on_off = 0 WHERE name = \'" + containerName + "\' AND gpu_address = \'" + gpu + "/'",
                  function(err, results, fields) {
                      if (err) throw err;
                  })
              conn.end(function(err) {
    ...
    setContainerOn: function(containerName, gpu, callback) {
      ...
      conn.connect(function(err) {
          if (err) {
              ...
              conn.query("UPDATE containers_info SET on_off = 1 WHERE name = \'" + containerName + "\' AND gpu_address = \'" + gpu + "/'",
                  function(err, results, fields) {
                      if (err) throw err;
                  })
              conn.end(function(err) {
